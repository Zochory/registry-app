...
function buildRegistryItemUrl(name: string, baseUrl: string = DEFAULT_REGISTRY_BASE_URL) {
  ...
  return `${normalizedBase}/${normalizedName}.json`
}
...
export function OpenInV0Button({
  ...
  const registryItemUrl = buildRegistryItemUrl(name)
  const openInV0Url = `https://v0.dev/chat/api/open?url=${encodeURIComponent(registryItemUrl)}`
